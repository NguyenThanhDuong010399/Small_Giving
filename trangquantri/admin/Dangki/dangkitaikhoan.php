...
if($Email!=''&&$SDT!='')
{
    $sql="SELECT TenNguoiDung from tbl_nguoi_dung where Email='$Email' or SDT='$SDT'";
    ...
    else
    {
        ...
        {
            $sql1="INSERT INTO tbl_nguoi_dung(Email,SDT,TenNguoiDung,MatKhau,idNhom) VALUES('$Email','$SDT','$TenNguoiDung','$MatKhau','$nhahaotam')";
            $stmt_insert=$conn->prepare($sql1);
            // $stmt->bindValue(':Email', htmlspecialchars(strip_tags($Email)),PDO::PARAM_STR);
            // $stmt->bindValue('', htmlspecialchars(strip_tags($data->SoDuTK)),PDO::PARAM_STR);
            // $stmt->bindValue(':SoNguoiTG', htmlspecialchars(strip_tags($data->SoNguoiTG)),PDO::PARAM_STR);
