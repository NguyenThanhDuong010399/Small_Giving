...
if(isset($data->idHoatDong)){
    ...
    $get_post = "SELECT * FROM tblhoatdong WHERE idHoatDong=:post_id";
    ...
    if($get_stmt->rowCount() > 0){
        ...
        $post_sodu = isset($data->SoDuTK) ? $data->SoDuTK : $row['SoDuTK'];
        // $post_body = isset($data->body) ? $data->body : $row['body'];
        // $post_author = isset($data->author) ? $data->author : $row['author'];
        
        $update_query = "UPDATE tblhoatdong SET SoDuTK = :SoDuTK
        WHERE idHoatDong = :id";
        
        $update_stmt = $conn->prepare($update_query);
        
        // DATA BINDING AND REMOVE SPECIAL CHARS AND REMOVE TAGS
        $update_stmt->bindValue(':SoDuTK', htmlspecialchars(strip_tags($post_sodu)),PDO::PARAM_STR);
